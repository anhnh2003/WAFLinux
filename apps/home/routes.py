...
def parse_log_line(line):
    ...
    # 2024-12-22T23:24:59.573555-05:00 kali kernel: OUTPUT LOG: IN= OUT=eth0 SRC=10.0.2.128 DST=10.0.2.1 LEN=40 TOS=0x10 PREC=0x00 TTL=64 ID=34687 DF PROTO=TCP SPT=22 DPT=55616 WINDOW=697 RES=0x00 ACK URGP=0
    pattern = re.compile(
        r'(?P<timestamp>\S+) (?P<hostname>\S+) kernel: (?P<chain>\w+): IN=(?P<in>\S*) OUT=(?P<out>\S*) MAC=(?P<mac>\S*) SRC=(?P<src>\S*) DST=(?P<dst>\S*) LEN=(?P<len>\d+) TOS=(?P<tos>\S*) PREC=(?P<prec>\S*) TTL=(?P<ttl>\d+) ID=(?P<id>\d+) DF PROTO=(?P<proto>\S*) SPT=(?P<spt>\d+) DPT=(?P<dpt>\d+) WINDOW=(?P<window>\d+) RES=(?P<res>\S*) ACK URGP=(?P<urgp>\d+)'
    )
